import React from 'react'

const Section9 = () => {
  return (
      <div>
          <div className="section-9">
              <div className="container">
                  <div className="h-p-box-10">
                      <h2 className='heading-13'>We provide services in these areas</h2>
                      <p className='para-15'>Lörem ipsum koda astrobel: sutaveligen. Rodod bänera viliga. Pregigt primasofi dede facebooka: förutom tivaligt. Fejkade</p>
                  </div>
                  <div className="flex-box-7">
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Knoxville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Sevierville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Maynardville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Knoxville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Sevierville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Maynardville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Knoxville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Sevierville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Maynardville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Knoxville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Sevierville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Maynardville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Knoxville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Sevierville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Maynardville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Knoxville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Sevierville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Maynardville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Knoxville</h3>
                      </div>
                      <div className="box-6">
                          <div className="svg">
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <circle cx="24" cy="24" r="24" fill="#FF0000" />
                                  <path d="M34.7754 19.5625C33.4629 13.7875 28.4254 11.1875 24.0004 11.1875C24.0004 11.1875 24.0004 11.1875 23.9879 11.1875C19.5754 11.1875 14.5254 13.775 13.2129 19.55C11.7504 26 15.7004 31.4625 19.2754 34.9C20.6004 36.175 22.3004 36.8125 24.0004 36.8125C25.7004 36.8125 27.4004 36.175 28.7129 34.9C32.2879 31.4625 36.2379 26.0125 34.7754 19.5625ZM24.0004 25.825C21.8254 25.825 20.0629 24.0625 20.0629 21.8875C20.0629 19.7125 21.8254 17.95 24.0004 17.95C26.1754 17.95 27.9379 19.7125 27.9379 21.8875C27.9379 24.0625 26.1754 25.825 24.0004 25.825Z" fill="white" />
                              </svg>
                          </div>
                          <h3 className='heading-14'>Sevierville</h3>
                      </div>
                  </div>
              </div>
          </div>
    </div>
  )
}

export default Section9